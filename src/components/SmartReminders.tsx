...
import { Idea, Log, Checklist, Review } from '../utils/apiService';
...
const getAudioContext = (): AudioContextType | null => {
  if (!audioContext && (window.AudioContext || (window as any).webkitAudioContext)) {
    audioContext = new ((window.AudioContext || (window as any).webkitAudioContext) as new () => AudioContextType)();
  }
  return audioContext;
};
...
const SmartReminders: React.FC<SmartRemindersProps> = ({ ideas, logs, checklist, reviews }) => {
  ...
  const [reminderHistory, setReminderHistory] = useLocalStorage<ReminderHistory[]>('neural-reminder-history', []);
                    ...
                    {reminder.dueDate && (
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          ...
                          {reminder.daysUntilDue !== null && reminder.daysUntilDue > 1 && `Due in ${reminder.daysUntilDue} days`}
                          {reminder.daysUntilDue !== null && reminder.daysUntilDue < 0 && `OVERDUE by ${Math.abs(reminder.daysUntilDue)} days`}
                        </span>
                      </div>
                    )}

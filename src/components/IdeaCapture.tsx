...
interface OrganizedTheme {
  ...
  description?: string;
  priority?: string;
  ideas: Array<string | { content: string; tags?: string[] }>;
}

interface OrganizedData {
  summary?: string;
  themes?: OrganizedTheme[];
  nextSteps?: string[];
}
...
interface IdeaCaptureProps {
  ...
  setOrganizedData: React.Dispatch<React.SetStateAction<OrganizedData | null>>;
...
export default function IdeaCapture({
  ...
  const classifyIdeaInBackground = async (idea: IdeaType): Promise<void> => {
    try {
      ...
      if (response.ok) {
        const classification: Classification = await response.json();
        ...
        setIdeas(prev => prev.map(i =>
          i.id === idea.id
            ? {
                ...
                duration: classification.duration || null,
                recurrence: classification.recurrence || 'none',
                timeOfDay: classification.timeOfDay || null,
  ...
  const classifyIdea = async (content: string, context: string, tags: string[]): Promise<Classification | null> => {
    try {
      const response = await fetch('http://localhost:3001/api/classify-idea', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  }, [currentIdea, context, selectedTags, ideas, setIdeas]);
  ...
  // Modal handlers
  const openEditModal = useCallback((idea: IdeaType): void => {
    setEditingIdea(idea);
  }, []);

  const handleSaveFromModal = (updatedIdea: IdeaType): void => {
    setIdeas(prev => prev.map(i =>
  ...
  const copyIdea = useCallback(async (idea: IdeaType): Promise<void> => {
  ...
  const handleOrganizeIdeas = async (): Promise<void> => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setOrganizationError(result.error || 'Failed to organize ideas');
      }
    } catch (error) {
  ...
  const groupedIdeas = useMemo(() => {
    ...
    const groups: Record<string, IdeaType[]> = {
      today: [],
      yesterday: [],
      thisWeek: [],
      thisMonth: [],
  ...
  // Toggle filter
  const toggleFilter = useCallback((type: 'tags' | 'classifications', value: string): void => {
    ...
    <div className="space-y-6">
      ...
      <div className="neural-card pulse-glow">
        ...
        <textarea
          ref={textareaRef}
          value={currentIdea}
          onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setCurrentIdea(e.target.value)}
                            ...
                            {theme.ideas.map((idea, ideaIndex) => (
                              ...
                              >
                                <p className="text-gray-200">
                                  {typeof idea === 'string' ? idea : idea.content}
                                </p>
                                {typeof idea !== 'string' && idea.tags && idea.tags.length > 0 && (

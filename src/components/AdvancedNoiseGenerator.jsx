          ...
          if (elapsedMs >= delayDurationMs) {
            ...
            if (noiseGeneratorRef.current) {
              try {
                ...
                if (gammaEnabledRef.current && nextVariationNumber % 1800 === 0) {
                  console.log(`🌊 Refreshing gamma wave at variation ${nextVariationNumber} (~1 hour mark)`);
                  noiseGeneratorRef.current.stopGammaWave();
                  noiseGeneratorRef.current.startGammaWave(
                    gammaCarrierFreqRef.current,
                    40,
                    gammaVolumeRef.current
                  );
                  console.log('✅ Gamma wave refreshed (~1-2ms gap, pink/brown uninterrupted)');
                }
              } catch (error) {
        ...
        <div className="space-y-6">
          ...
          <div className="bg-neural-darker/50 border border-gray-800 rounded-lg p-4">
            ...
            <ul className="text-sm text-gray-400 space-y-1">
              ...
              <li>• <strong>Gamma Auto-Refresh:</strong> If gamma is enabled, it automatically refreshes every ~1 hour (~1-2ms gap, pink/brown uninterrupted)</li>

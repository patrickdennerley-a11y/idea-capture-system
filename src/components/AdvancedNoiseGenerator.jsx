...
export default function AdvancedNoiseGenerator({ audioContextRef, activeSession, setActiveSession }) {
  ...
  // Update overlay gamma carrier ref when changed
  useEffect(() => {
    overlayGammaCarrierRef.current = overlayGammaCarrier;
  }, [overlayGammaCarrier]);

  // Update overlay gamma enabled ref when changed
  useEffect(() => {
    overlayGammaEnabledRef.current = overlayGammaEnabled;
  }, [overlayGammaEnabled]);
      ...
      setActiveSession(prev => {
        ...
        if (prev.inDelay) {
          ...
          if (elapsedMs >= delayDurationMs) {
            ...
            // Auto-refresh check BEFORE playing (prevent AudioContext corruption on long sessions)
            // This ensures we refresh BEFORE starting new audio, avoiding race conditions
            ...
            if (autoRefreshEnabled && refreshInterval > 0) {
              ...
              if (variationsSinceLastRefresh >= refreshInterval) {
                ...
                // Stop current audio and gamma
                if (noiseGeneratorRef.current) {
                  noiseGeneratorRef.current.stop();
                  noiseGeneratorRef.current.stopGammaWave();
                }

                // Recreate AudioContext synchronously
                if (audioContextRef.current) {
                  try {
                    audioContextRef.current.close();
                    console.log('  ✅ Old AudioContext closed for refresh');
                  } catch (e) {
                ...
                audioContextRef.current = new (window.AudioContext || window.webkitAudioContext)();
                noiseGeneratorRef.current = new NoiseGenerator(audioContextRef.current);
                console.log('  ✅ AudioContext refreshed');
              }
            }

            // Play new noise (with fresh AudioContext if refresh just happened)
            if (noiseGeneratorRef.current) {
              try {
                if (nextType === 'gamma') {
                ...
                // Restart overlay gamma ONLY if we just refreshed (maintain continuous playback otherwise)
                if (didRefresh && overlayGammaEnabledRef.current) {
                  noiseGeneratorRef.current.startGammaWave(
                    overlayGammaCarrierRef.current,
                    40,
                    overlayGammaVolumeRef.current
                  );
                  console.log(`  ✅ Overlay gamma restarted after refresh (${overlayGammaCarrierRef.current}Hz, ${overlayGammaVolumeRef.current}%)`);
                }
              } catch (error) {
    ...
    setActiveSession(prev => {
      ...
      if (prev.isPaused) {
        ...
        if (noiseGeneratorRef.current && prev.currentVariation) {
          // Resume the current variation (check if gamma or noise type)
          if (prev.currentType === 'gamma') {
            const gammaCarrier = prev.currentVariation.parameters?.carrierFreq || alternatingGammaCarrierRef.current;
            const gammaDuration = prev.currentVariation.durationMinutes;
            noiseGeneratorRef.current.playGamma(
              gammaCarrier,
              40,
              alternatingGammaVolumeRef.current,
          ...
          // Resume overlay gamma if enabled (use ref for current user setting)
          if (overlayGammaEnabledRef.current) {
            noiseGeneratorRef.current.startGammaWave(
              overlayGammaCarrierRef.current,
              40,
              overlayGammaVolumeRef.current
            );
            console.log('  ✅ Overlay gamma resumed after unpause');
          }
        }
  ...
  const performAutoRefresh = useCallback(async () => {
    ...
    // OVERLAY GAMMA BUG FIX: Check if overlay gamma is ENABLED (user setting)
    // instead of just checking if it was playing at this exact moment
    ...
    console.log('  Overlay gamma enabled:', overlayGammaWasEnabled);
    ...
    if (activeSession && !activeSession.completedAt && !activeSession.isPaused && activeSession.currentVariation) {
      try {
        ...
        if (overlayGammaWasEnabled) {
          ...
          console.log(`  ✅ Resumed overlay gamma wave (${overlayGammaCarrierValue}Hz, ${overlayGammaVolumeValue}%)`);
        }
      } catch (error) {
  ...
  const forceRestartAudio = useCallback(async () => {
    ...
    // OVERLAY GAMMA BUG FIX: Use ref values
    const overlayGammaWasEnabled = overlayGammaEnabledRef.current;
    const overlayGammaCarrierValue = overlayGammaCarrierRef.current;
    const overlayGammaVolumeValue = overlayGammaVolumeRef.current;
    ...
    if (activeSession && !activeSession.completedAt) {
      const wasGammaEnabled = activeSession.gammaWasEnabled || overlayGammaWasEnabled;

...
import { Log, Checklist } from '../utils/apiService';
...
export default function EndOfDayReview({ reviews, setReviews, logs, checklist }: EndOfDayReviewProps) {
  ...
  const saveReview = (): void => {
    ...
    const actualAvgEnergy = todayLogs.length > 0
      ? Math.round(todayLogs.reduce((sum, log) => sum + (log.energy || 0), 0) / todayLogs.length)
      : 0;

    const actualStudyTime = todayLogs.filter(l => l.activity === 'Studying').length * 0.5;

    const completionRate = (checklist as any).items
      ? ((checklist as any).items.filter((i: any) => i.completed).length / (checklist as any).items.length) * 100
      : 0;
  ...
  const generateInsights = (
    ...
    if (studySessions.length > 0) {
      const avgStudyEnergy = Math.round(
        studySessions.reduce((sum, l) => sum + (l.energy || 0), 0) / studySessions.length
      );
      insights.push({
        type: 'info',
      ...
      {showReview && !hasCompletedToday && (
        <div className="space-y-4 animate-slide-in">
          ...
          <div>
            ...
            <textarea
              value={dayOverall}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setDayOverall(e.target.value)}
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setAccomplishments(e.target.value)}
          ...
          <div>
            ...
            <input
              ...
              max="10"
              value={energyRecall}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEnergyRecall(parseInt(e.target.value))}
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setTomorrowPriorities(e.target.value)}
              placeholder="1. ...&#10;2. ...&#10;3. ..."
              className="neural-textarea min-h-[100px]"
            />
          </div>

...
interface CalendarEvent {
  ...
  title: string;
  description: string;
  start: string; // ISO string when stored
  end: string;   // ISO string when stored
  priority: Priority;
}
...
export default function CalendarView({ routineToLoad = null }: CalendarViewProps) {
  ...
  const [editingEvent, setEditingEvent] = useState<CalendarEventWithDates | null>(null);
  ...
  const calendarEvents = useMemo((): CalendarEventWithDates[] => {
  ...
  const handleLoadRoutine = (): void => {
    ...
    try {
      ...
      const newEvents: CalendarEvent[] = routineToLoad.schedule.map((block, index) => {
        ...
        return {
          ...
          priority: (block.priority?.toLowerCase() || 'medium') as Priority,
        };
      });
  ...
  const handleSelectEvent = (event: CalendarEventWithDates): void => {
  ...
  const handleEventDrop = ({ event, start, end }: EventInteractionArgs<CalendarEventWithDates>): void => {
    saveUndo();
  ...
  const handleEventResize = ({ event, start, end }: EventInteractionArgs<CalendarEventWithDates>): void => {
    saveUndo();
    const updatedEvent: CalendarEvent = {
      ...event,
      start: start.toISOString(),

...
interface RoutineGeneratorProps {
  ...
  smartRoutines: SmartRoutine[];
  setSmartRoutines: React.Dispatch<React.SetStateAction<SmartRoutine[]>>;
  ...
  smartRoutinesMetadata: SmartRoutinesMetadata | null;
  setSmartRoutinesMetadata: React.Dispatch<React.SetStateAction<SmartRoutinesMetadata | null>>;
  smartRoutineStates: Record<string, string>;
  setSmartRoutineStates: React.Dispatch<React.SetStateAction<Record<string, string>>>;
...
export default function RoutineGenerator({
  ...
  const [currentView, setCurrentView] = useState<string>('generator'); // 'generator' or 'calendar'
  const [savedRoutines, setSavedRoutines] = useLocalStorage<GeneratedRoutine[]>('neural-saved-routines', []);
  ...
  const handleSaveConfirm = (): void => {
    ...
    setSavedRoutines(prev => [saved, ...prev]);
  ...
  const handleLoad = (routine: GeneratedRoutine): void => {
                ...
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    ...
                    <div className="text-xs text-gray-500">
                      Saved {routine.savedAt ? new Date(routine.savedAt).toLocaleString() : ''}
                    </div>
                  </div>
          ...
          {generatedRoutine.schedule && generatedRoutine.schedule.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {generatedRoutine.schedule.map((block, index) => (
                  <div
                    key={index}
                    className="bg-neural-darker border border-gray-800 rounded-lg p-4 hover:border-neural-purple transition-colors"
                  >
          ...
          {generatedRoutine.energyTips && generatedRoutine.energyTips.length > 0 && (
            <div className="bg-neural-darker border border-gray-800 rounded-lg p-4">
              ...
              <ul className="space-y-2">
                {generatedRoutine.energyTips.map((tip, index) => (

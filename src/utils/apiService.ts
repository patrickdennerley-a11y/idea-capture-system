  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (
        ...
        throw new Error(
          "Backend server is not responding. Please make sure it's running on port 3001."
        );
      }
 ...
 */
export const organizeIdeas = async (
  ideas: Idea[]
): Promise<ApiResponse<unknown>> => {
  try {
    const response = await fetchWithRetry<unknown>(
      `${API_BASE_URL}/api/organize-ideas`,
      {
        method: 'POST',
...
export const getWeeklySummary = async (
  ...
  try {
    const response = await fetchWithRetry<unknown>(
      `${API_BASE_URL}/api/weekly-summary`,
      {
        method: 'POST',
        body: JSON.stringify({ ideas }),
      }
...
export const analyzePatterns = async (
  ...
  try {
    const response = await fetchWithRetry<unknown>(
      `${API_BASE_URL}/api/analyze-patterns`,
      {
        method: 'POST',
 ...
 */
export const classifySubject = async (
  subject: string
): Promise<ApiResponse<unknown>> => {
  try {
    const response = await fetchWithRetry<unknown>(
      `${API_BASE_URL}/api/classify-subject`,
      {
        method: 'POST',
...
export const getPlanningAdvice = async (
  ...
  try {
    const response = await fetchWithRetry<unknown>(
      `${API_BASE_URL}/api/plan-activity`,
      {
        method: 'POST',
...
export const generateDailyRoutine = async (
  ...
  try {
    const response = await fetchWithRetry<unknown>(
      `${API_BASE_URL}/api/generate-routine`,
      {
        method: 'POST',

...
function App() {
  ...
  const [showIconCustomizer, setShowIconCustomizer] = useState<boolean>(false);

  // State with localStorage persistence
  const [ideas, setIdeas] = useLocalStorage<IdeaType[]>('neural-ideas', []);
  const [logs, setLogs] = useLocalStorage<ActivityLog[]>('neural-logs', []);
  const [reviews, setReviews] = useLocalStorage<Review[]>('neural-reviews', []);
  const [checklist, setChecklist] = useLocalStorage<Checklist>('neural-checklist', {
    date: getTodayString(),
    items: [],
  });
  ...
  const [smartRoutines, setSmartRoutines] = useState<SmartRoutine[]>([]);
  const [isGeneratingSmartRoutines, setIsGeneratingSmartRoutines] = useState<boolean>(false);
  const [smartRoutinesError, setSmartRoutinesError] = useState<string | null>(null);
  const [showSmartRoutines, setShowSmartRoutines] = useState<boolean>(false);
  const [smartRoutinesMetadata, setSmartRoutinesMetadata] = useState<SmartRoutinesMetadata | null>(null);
  const [smartRoutineStates, setSmartRoutineStates] = useState<Record<string, string>>({});

  // History for all AI generations
  const [generationHistory, setGenerationHistory] = useLocalStorage<GenerationHistory>('neural-generation-history', {
    routines: [],
    smartRoutines: [],
    ideas: [],
    plans: []
  ...
  const [activeSession, setActiveSession] = useState<NoiseSession | null>(null);
  ...
  const renderActiveTab = useCallback(() => {
    switch (activeTab) {
      ...
      default:
        return <IdeaCapture ideas={ideas} setIdeas={setIdeas} isOrganizing={isOrganizing} setIsOrganizing={setIsOrganizing} organizedData={organizedData} setOrganizedData={setOrganizedData} organizationError={organizationError} setOrganizationError={setOrganizationError} showOrganized={showOrganized} setShowOrganized={setShowOrganized} />;
    }
  }, [
    activeTab,
    ideas,
